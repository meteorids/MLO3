...
%  modify configuration files
   ...
   % Step 2: modify atmosphere profile configuration
    fid = fopen('./TESTozoneMaterials/earth_atmospheric_gases');
    n=1;
    while(1)
        lin = fgetl(fid);
        if ~ischar(lin), break, end
    ...
    fid = fopen('./TESTozoneMaterials/earth_atmospheric_gases','w');
    for k=1:length(l)
        fprintf(fid,'%s\n',l{k});
    end
    fclose(fid);
    ...
    fid = fopen('./TESTozoneMaterials/cloud');
    n=1;
    while(1)
        lin = fgetl(fid);
        if ~ischar(lin), break, end
    ...
    fid = fopen('./TESTozoneMaterials/cloud','w');
    for k=1:length(l)
        fprintf(fid,'%s\n',l{k});
    end
    fclose(fid);
    ...
    % run AccuRT
    system('AccuRT TESTozone');

    % read iop, aod and irradiance
    % rad=readRadiance('./NNOutput/radiance.txt');
    irrad=readIrradiance('./TESTozoneOutput/cosine_irradiance_total_downward.txt');

    iop=readIops('./TESTozoneOutput/iops.txt');
    ...
    irrad1=[solz vf ozone irrad.irradiance(2,:)];
