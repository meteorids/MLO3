...
c     instrument you must use CONVLOG2.0 to convert the files
c     to the required file names and format
      ...
      character calfile*13, serial*3, no(5), yr*2
      ...
      character*12  uintname(20),uintname2(20)
...
c     read  site info
 ...
 601  continue
      ...
      read(1,*)
      read(1,*) irato3
      if(irato3 .lt. 1 .or. irato3.gt.2) irato3=1
      read(1,*)
      read(1,*) iauto3
...
c     read calibration file given in site.inp
 ...
 499  continue
      ...
      do i = 1,nact
        read(1,9) actname(i)
        read(1,30) comment(i)
        read(1,12) uintname(i)
        read(1,12) uintname2(i)
...
c     in subroutine deriveozone )
      do ii = 1,36 
         do jj = 0,95
            read(1,*) ioz,iz,r32L,r31L,r32M,r31M,r32H,r31H
...
C averages are calculated over a time interval around the time 'hnoon'.
...
c (hnoon-tdiff, hnoon+tdiff)
 ...
 35   continue
      ...
      call skipline(20)
      write(6,*) '        NILU-UV Data Processing Program V2.0beta'
      call skipline(5)
...
c     ############################################
c     open a new file to store channel data
c     ############################################
c      open(88,file='output_channels_'//century//yr
c     $  //chmon(mon)//chday(id)//'.'//serial)
c     ############################################     
...
c automaticallys switch between ch3/ch1 and ch3/ch2 
     ...
     $   iauto3.eq.2 .and. z.lt.zswitcho3pair) then 
             ratio =(1.0 - r300/rawcal1)*f320/f300
             call deriveozone(RATIO,z,x,Nb)
         end if

         if(f305.gt.0.0 .and. f320.gt.0.0 .and. 
     $   iauto3.eq.2 .and. z.ge.zswitcho3pair) then 
             ratio =(1.0 - r305/rawcal2)*f320/f305
             call deriveozone(RATIO,z,x,Na)
         end if
         ...
         if(idetail.eq.2) then
        write(3,200) iym,im,id,jul,ih,min,is,z,drate,clt,x   
         end if
         
c      ##################################################
c      write channel data to files
c      ##################################################
c       write(88,'(F6.2,F6.2,F6.2,F6.2,F6.2,F6.2)') z,f300,f305,f320,f340,f380
c      ##################################################        
...
22         continue
 ...
 30   format(a30)
 70   format(' Cannot find calibration file:  ',a13)
 100  format(i2,i2,i2,1x,i2,i2,i2,6e10.3,f6.2)
 110  format(28x,'NILUUV ',a3)
 111  format('Uvpro Version 2.0')
...
c     $ f6.1,2x,E11.4,2x,f7.2)
 ...
 422  format('Calibration file: ',a13)
 ...
 428  format('Ozone scale factor used: ',f5.1)
 429  format('CLT scale factor used: ',f5.1)
 430  format('                 <------------ averages ------------->',
     $'   Maximum    Integrated')
...
c ----------------------------------------------------------
 ...
 1    read(1,*,end=99,err=999) iyy,j,
     $r1,r2,r3,r4,r5,r6
      ...
      t(k) = iyy*1.0+j/365.0
	if(k.gt.1 .and. t(k).eq.t(k-1)) t(k) = t(k-1) + 0.01
...
c    check if the first drift factors are close to 1.000     
c      do i = 1,6
c         if(abs(dr(1,i) -1.0) .gt.0.001) go to 800
c      end do

c     check if drift factors are zero or negative
	do n = 1,k-2
      do i = 1,6
         if(dr(n,i).le.0.0) go to 810
      end do
	end do
	
c     check if calibration dates are listed in chronological order
	   do n = 1,k-2
         if(t(n+1) .lt. t(n)) go to 820
	   end do		


c     compute drift factors for the present day
      do jj = 1,6
      do n=1,k-2
 ...
 996  continue
      ...
      write(1,100)
      write(1,110) iiy,iim,iid,ijul
 100  format('year mm dd  dayno     ch1     ch2     ch3'
     $,'     ch4     ch5     ch6')
 110  format(i4,1x,i2,1x,i2,4x,i3,
 ...
 800	do i = 1,6
	...
	write(6,200) serial
	write(6,*) 
     $' All drift factors in line 2 must be equal to 1.000'
      do i = 1,3
            write(6,*)
      end do
...
810	do i = 1,6
	...
	write(6,200) serial
	write(6,*) ' Zero or negative drift factors found!'
      do i = 1,3
            write(6,*)
      end do
      write(6,*) 'Hit Enter-key to abort program'
...
820	do i = 1,6
	...
	write(6,200) serial
	write(6,*) 
     $'Calibration dates must be listed in chronological order!'
      do i = 1,3
            write(6,*)
      end do
      write(6,*) 'Hit Enter-key to abort program'
